...
- **P0 Critical (10+ personas or blocker):** 8 — **5 ✅ DONE 2026-04-28** (#1, #2, #3, #5, #6)
...
| B5  | Flow tab (canvas)  | **30+ React "Maximum update depth exceeded" errors** in console during normal interaction — infinite re-render loop.                             | 4/15 (Alex, Marcus, David, Karen)                                     |
| B6  | Flow tab (canvas)  | **Palette drawer overlays canvas and eats pointer events** on blocks underneath (Playwright captured "Block library intercepts pointer events"). | 7/15 (Alex, Marcus, Karen, Tom, Priya, Mike, David)                   |
| B7  | Simulator          | **"Send" button permanently disabled** in default state — exploration had to fall back to "Run" pill.                                            | 6/15 (Alex, Marcus, Karen, Mike, Yuki, Betty)                         |
...
| 7                    | Flow tab (canvas)  | Feedback & State               | 30+ React render-loop errors during normal interaction — "Maximum update depth exceeded."                              | 4/15 (Alex, Marcus, Karen, David)                                                            | Audit `index.tsx:131-152` Toast `useEffect` deps and `flow-draft-sync.tsx`. Wrap inline callbacks in `useCallback`.                                                                                                                                                                                                        |
...
1. **Triage the 12 blockers first** — most are missing primary controls (kill switch, support, rollback, search) that no fix-up of polish can substitute for.
2. **Tackle the P0 chrome leaks together as one PR** — "Saved to Supabase," `setter-v2`, `src/lib/prompts/sections/*.ts`, the "DEV" badge, "until flow_id lands on the conversations table." These are 5 string changes that unblock 6 of the 8 P0 issues. Highest-leverage cleanup in the report.
3. **Kill the render loop** (P0 #7) before any other code change — it compounds with everything else.
4. **Fix the palette overlay click-eating** (B6) — interaction bug, not polish.
