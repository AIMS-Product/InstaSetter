...
function pickMotivationFromTags(tags: string[] | null | undefined): string | undefined {
...
function pickLocationFromTags(tags: string[] | null | undefined): string | undefined {
  if (!tags) return undefined
  return tags
...
function pickBudgetFromTags(tags: string[] | null | undefined): string | undefined {
  if (!tags) return undefined
  return tags.find((t) => t.startsWith('budget:'))?.replace('budget:', '')
}

function trimToCloseLimit(value: string | undefined | null): string | undefined {
...
export function buildCloseLeadPayload(
  ...
  const conversationUrl = buildConversationUrl(input.appBaseUrl, conversation.id)
