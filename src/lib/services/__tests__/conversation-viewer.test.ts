...
function createMockClient({ conversations, leads }: MockClientArgs): {
  ...
  const client = {
    from: vi.fn((table: string) => {
      ...
      if (table === 'conversations') return makeConversationsChain(conversations, log)
...
function makeLeadsChain(leads: SeedLead[], log: MockCallLog) {
  ...
  const chain: Record<string, unknown> = {
    ...
    or: vi.fn((expr: string) => {
      ...
      filters.push((lead) => {
        if (
          ['sent', 'failed', 'failed_permanent'].includes(lead.close_sync_status)
        ) {
          return true
        }
        if (lead.close_sync_status === 'pending' && lead.close_sync_attempts > 0) {
          return true
        }
        return false
      })
  ...
  const chain: Record<string, unknown> = {
    ...
    not: vi.fn((col: string, op: string, value: unknown) => {
      ...
      if (op === 'in' && typeof value === 'string') {
        const ids = value.replace(/^\(|\)$/g, '').split(',').filter(Boolean)
        filters.push(
          (conv) =>
            !ids.includes(
...
const FIXTURE_LEADS: SeedLead[] = [
  { conversation_id: 'conv-sent-1', close_sync_status: 'sent', close_sync_attempts: 1 },
  { conversation_id: 'conv-sent-2', close_sync_status: 'sent', close_sync_attempts: 1 },
  { conversation_id: 'conv-failed-1', close_sync_status: 'failed', close_sync_attempts: 2 },
  { conversation_id: 'conv-failed-perm-1', close_sync_status: 'failed_permanent', close_sync_attempts: 3 },
  {
  ...
  { conversation_id: 'conv-skipped', close_sync_status: 'skipped', close_sync_attempts: 0 },
...
describe('listConversations — closeSyncStatus filter', () => {
  ...
  it("'pending' excludes 0-attempt rows (queued but never tried)", async () => {
    ...
    expect(log.leadGt.some((c) => c.col === 'close_sync_attempts' && c.value === 0)).toBe(true)
  })
  ...
  it("'not_synced' returns conversations with no lead, skipped, or 0-attempt pending", async () => {
    ...
    expect(log.conversationNot.some((c) => c.col === 'id' && c.op === 'in')).toBe(
      true
    )
  ...
  it.each<{
    ...
    async ({ filter, expectedIds }) => {
      setup()
      const rows = await listConversations({ closeSyncStatus: filter, limit: 2 })
