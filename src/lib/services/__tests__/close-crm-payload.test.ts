...
describe('buildConversationUrl', () => {
  it('joins the base url to the conversation path', () => {
    expect(buildConversationUrl('https://insta-setter.vercel.app', 'abc')).toBe(
      'https://insta-setter.vercel.app/dashboard/conversations/abc'
    )
  })
...
describe('buildCloseLeadPayload', () => {
  ...
  it('omits attribution custom fields when no attribution row is present', () => {
    const payload = buildCloseLeadPayload(baseInput({ attribution: null }))
