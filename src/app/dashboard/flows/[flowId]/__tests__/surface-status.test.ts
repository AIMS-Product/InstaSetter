...
describe('surface status copy', () => {
  it('marks dirty drafts as autosaved and not live', () => {
    const status = getDraftWorkspaceStatus(true)

    expect(status.label).toBe('Draft autosaved')
    expect(String(status.detail)).toContain('shared Supabase draft')
  ...
  it('treats clean drafts as having no pending editor changes', () => {
    ...
    expect(status.label).toBe('No draft changes')
    expect(String(status.detail)).toContain('shared draft is saved')
  })
